...
final public class OAuthPublisher {
    public class Statuses: Twitter {
        /// Updates the authenticating user's current status, also known as Tweeting.
        public func update(
            status: String,
            inReplyToStatusId: String? = nil
        ) -> AnyPublisher<Response.Status, AFError> {
            super.publish(Update(status: status, inReplyToStatusId: inReplyToStatusId))
        }
        
        /// Destroys the status specified by the required ID parameter.
        public func destroy(id: String, trimUser: Bool) -> AnyPublisher<Response.Status, AFError> {
            publish(Destroy(id: id, trimUser: trimUser))
        }
        ...
        public func show(
            ...
            publish(Show(id: id, trimUser: trimUser, includeMyRetweet: includeMyRetweet, includeEntities: includeEntities, includeExtAltText: includeExtAltText, includeCardUri: includeCardUri))
        }
        ...
        -> AnyPublisher<[Response.Status], AFError> {
            publish(Lookup(id: id, includeEntities: includeEntities, trimUser: trimUser, map: map, includeExtAltText: includeExtAltText, includeCardUri: includeCardUri))
        }
        ...
        public func retweet(
            ...
            publish(Retweet(id: id, trimUser: trimUser))
        }
        ...
        public func unretweet(
            ...
            publish(Unretweet(id: id, trimUser: false))
        }
        ...
        public func retweets(
            ...
            publish(Retweets(id: id, count: count, trimUser: trimUser))
        }
        ...
        public func retweetsOfMe(
            ...
            publish(RetweetofMe(count: count, sinceId: sinceId, maxId: maxId, includeEntities: includeEntities, includeUserEntities: includeUserEntities))
        }
        ...
        -> AnyPublisher<Response.Status, AFError> {
            publish(Retweeters(id: id, count: count, cursor: cursor, stringfyIds: stringfyIds))
        }
        ...
        public func homeTimeline(
            ...
            publish(Timeline.Home(count: count))
        }
        ...
        public func userTimeline(
            ...
            publish(Timeline.User(screenName: screenName, count: count))
        }
        ...
        public func mentionsTimeline(
            ...
            publish(Timeline.Mentions(count: count))
        }
    }
    
    public class Favorites: Twitter {
    ...
    public class Account: Twitter {
        public func getUserCredentials() -> AnyPublisher<Response.User, AFError> {
            publish(Credential(includeEntities: false, skipStatus: true, includeEmail: false))
        }
    }
}
